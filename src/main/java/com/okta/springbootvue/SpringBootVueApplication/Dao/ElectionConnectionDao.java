...
public class ElectionConnectionDao {
	...
	public void insertPolicy(Policy policy){
		...
		stmt.setInt(5,policy.getWriteIn());
	...
	public void updatePolicy(Policy policy){
		try {
			Connection conn = connectionDao.RetrieveConnection();
		String sql = "UPDATE electionPolicy SET electionID=?, type=?, frequency=?, num_votes=?, writeIn=?, abstain=? WHERE electionID=?";
		PreparedStatement stmt=conn.prepareStatement(sql);
